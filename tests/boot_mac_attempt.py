...
CDB_SHA = "4b96da50804a02015ebeb047911db3f8445e88279364905b3e2f7e2534656989"
...
OBJECT_SHA = "378a7e01aa4de46b73a000b7075ac2bb1ca39d56a947c2d3722e6d1cd604549f"
...
def listing_identity(listings):
    return sha("".join(m+"\n"+listing_serialized(listings[m]) for m in MODULES).encode("ascii"))
...
def caller(symbols):
    return {name: (symbols["_mac_attempt_test_"+name], size) for name, size in CALLER_SIZES.items()}


def verify(image, symbols, debug, memory, listings, objects):
    require(SIZE <= 32768 and sha(code_bytes(image, SIZE)) == CODE_SHA, "Attempt complete CODE")
    require(sha(debug.encode("ascii")) == CDB_SHA, "Attempt complete raw CDB")
    require(map_identity(symbols) == MAP_SHA and sha(memory.encode("ascii")) == MEM_SHA,
            "Attempt complete map/memory accounting")
    require(set(listings) == set(objects) == set(MODULES), "Attempt exact composition")
    ...
    allocated = verify_component_layout(image, symbols, debug, memory, "mac_attempt_test_result",
        tuple(m+".c" for m in MODULES), xdata_budget=2048)
    require(symbols["l_XSEG"] == XDATA and symbols["s_SSEG"] == STACK, "Attempt XDATA/IRAM ABI")
    ...
    require(set().union(*(spans[m] for m in MODULES[:5])) == set(range(shared)) and
            spans["mac_radio"] == set(range(shared, radio_end+1)) and
            spans["mac_attempt"] == set(range(radio_end+1, end+1)) and
            spans["test_mac_attempt"] == set(range(end+1, runtime)), "Attempt whole private/caller prefix")
    require(final+1 == XDATA and final-runtime == 11 and
    ...
    used = set()
    for name, (a, size) in caller(symbols).items():
        region = set(range(a, a+size))
        require(not used & region and region <= spans["test_mac_attempt"], "Attempt caller ABI: "+name)
        used.update(region)
    require(used == spans["test_mac_attempt"], "Attempt unaccounted caller storage")
    ...
    for module, names in (
        ...
        ("test_mac_attempt", ("mac_attempt_init", "mac_attempt_prepare", "mac_attempt_run",
...
def negatives(image, symbols, debug, memory, listings, objects):
    ...
    reject(image=image | {SIZE: 0}); reject(image={a: v for a, v in image.items() if a != SIZE-1})
    ...
    for step, first, final, current in checks:
        for i, (kind, address, value) in enumerate(step["events"]):
            ...
            require(kind == k and a in (None, address),
                    f"Attempt MMIO pc{pc:04x}, event{i}: expected{kind}/{address:04x}, got{k}/{a}")
        ...
        require(ram[0x1e00:0x1e08] == b"MAT1\x01\x08\0\0", "Attempt status reservation ABI")
        ...
        require(iram[0x7d:] == b"\xc7"*131 and sfr[1] == STACK+1,
        ...
        require(all(v == current.get(a, 0x69) for a, v in enumerate(hardware, 0x6000)),
                "Attempt unplanned XREG/FIFO write")
...
def load(output):
    path = output/"mac_attempt_test.ihx"
    raw_debug = path.with_suffix(".cdb").read_bytes()
    require(sha(raw_debug) == CDB_SHA, "Attempt complete raw CDB before decoding")
    return (path, parse_ihex(path.read_text()), parse_symbols(path.with_suffix(".map").read_text()),
            ...
            {m: (output/f"mac_attempt_test.{m}.rst").read_text() for m in MODULES},
            {m: (output/f"ma_{m}.rel").read_text() for m in MODULES})
...
def main():
    ...
    path, image, symbols, debug, memory, listings, objects = load(args.output)
    ...
    require(count == NEGATIVES, "Attempt artifact negative inventory")
    ...
    for n in range(CASES):
        vector = json.loads(subprocess.check_output([str(args.output/"host-mac-attempt-tests"),
    ...
    require((calls, events, sampled, peak) == INVENTORY, "Attempt replay/stack inventory")
    print(f"MAC attempt: {CASES} sequences/{calls} genuine calls/{events} MMIO events; "
          f"{SIZE}/32768 CODE, {XDATA}+64/2048 XDATA; SP{peak:02X}/7C; "
