...
"""Execute the standalone R22 NWK Data codec tests without radio or USB."""
...
def main():
    ...
    path = args.output / "nwk_frame_test.ihx"
    ...
    allocated = verify_component_layout(
        ...
        "nwk_frame_test_result", ("nwk_frame.c", "test_nwk_frame.c"),
    )
    stop = symbols["_nwk_frame_test_done"]
    require(image.get(stop) == 0
            and all(symbols[name] in image for name in ("_nwk_frame_decode", "_nwk_frame_encode")),
            "NWK frame codec/checkpoint is absent or changed")
    ...
    require(ram[STATUS_ADDRESS:STATUS_ADDRESS + 6] == b"NWF1\x01\x08", "NWK frame result ABI mismatch")
    failure = int.from_bytes(ram[STATUS_ADDRESS + 6:STATUS_ADDRESS + 8], "little")
    require(failure == 0, f"SDCC NWK frame test failed at C source line {failure}")
    require(all(value == 0xa5 for address, value in enumerate(ram) if address not in allocated),
            "NWK frame test wrote outside allocated XDATA")
    require(sfr[1] == symbols["s_SSEG"] - 1 and iram[128:] == b"\xc7" * 128,
            "NWK frame test crossed the upper IRAM guard or failed to unwind")
    print(f"NWK frame: {len(image)} CODE bytes; strict linked layout, actual codec vectors "
