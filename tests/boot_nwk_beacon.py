...
"""Execute the standalone R22 Beacon decoder tests without radio or USB."""
...
def main():
    ...
    path = args.output / "nwk_beacon_test.ihx"
    ...
    allocated = verify_component_layout(
        ...
        "nwk_beacon_test_result", ("nwk_beacon.c", "test_nwk_beacon.c"),
    )
    stop = symbols["_nwk_beacon_test_done"]
    require(image.get(stop) == 0 and symbols["_nwk_beacon_decode"] in image,
            "NWK Beacon decoder/checkpoint is absent or changed")
    ...
    require(ram[STATUS_ADDRESS:STATUS_ADDRESS + 6] == b"NWB1\x01\x08", "NWK Beacon result ABI mismatch")
    failure = int.from_bytes(ram[STATUS_ADDRESS + 6:STATUS_ADDRESS + 8], "little")
    require(failure == 0, f"SDCC NWK Beacon test failed at C source line {failure}")
    require(all(value == 0xa5 for address, value in enumerate(ram) if address not in allocated),
            "NWK Beacon test wrote outside allocated XDATA")
    require(sfr[1] == symbols["s_SSEG"] - 1 and iram[128:] == b"\xc7" * 128,
            "NWK Beacon test crossed the upper IRAM guard or failed to unwind")
    print(f"NWK Beacon: {len(image)} CODE bytes; strict linked layout, actual decoder vectors "
