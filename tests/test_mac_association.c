...
static const MCU_CODE uint8_t explicit_pan[] = {
    0x23, 0xcc, 0x97, 0x34, 0x12,
    1, 2, 3, 4, 5, 6, 7, 8, 0x34, 0x12,
    0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18,
    2, 0x78, 0x56, 0
};
...
static uint8_t body[126], observation, expected, scenario, profile;
static uint32_t now;

#define STEP(c, n, e, o) (profile == 0 || scenario == 35 \
    ? mac_association_step(c, n, e, o) \
    : mac_association_step_rx(c, n, e, o, MAC_RX_R22_ASSOCIATION_RESPONSE))
...
static uint16_t corpus(void)
{
    ...
    for (profile = 0; profile < 3; profile++) {
        for (scenario = 0; scenario < 36; scenario++) {
            setup();
            ...
            case 25:
                CHECK(STEP(&ctx, 197, NULL, &observation) == MAC_ASSOCIATION_OK);
            ...
            case 33:
                ...
                CHECK(STEP(&ctx, now, &event, &observation) == MAC_ASSOCIATION_OK);
                CHECK(mac_association_take(&ctx, &record) == MAC_ASSOCIATION_OK);
                CHECK(mac_association_start(&ctx, &request, now) == MAC_ASSOCIATION_OK);
                CHECK(ctx.generation == 2);
                event.kind = MAC_ASSOCIATION_FRAME;
                expected = MAC_ASSOCIATION_STALE; break;
            case 34: body[3] ^= 1; expected = MAC_ASSOCIATION_MISMATCH; break;
            default: break;
            }
            if (profile == 2) {
                for (position = 25; position > 13; position--)
                    body[position + 1] = body[position - 1];
                body[13] = 0x34; body[14] = 0x12;
                body[0] &= (uint8_t)~MAC_FLAG_PAN_COMPRESSION;
                event.length += 2;
                if (scenario == 0) event.body = explicit_pan;
                if (scenario == 6) expected = MAC_ASSOCIATION_RESPONSE;
                if (scenario == 14) expected = MAC_ASSOCIATION_MALFORMED;
                if (scenario == 34) { body[3] ^= 1; body[13] ^= 1; }
                if (scenario == 35) expected = MAC_ASSOCIATION_MALFORMED;
            }
            observation = 0xa5;
            saved = ctx;
            if (scenario == 28) {
                CHECK(STEP(&ctx, now, &event, &observation) == MAC_ASSOCIATION_INVALID);
            ...
            if (expected == MAC_ASSOCIATION_RESPONSE || expected >= MAC_ASSOCIATION_EXPIRED) {
                ...
                saved = ctx;
                CHECK(STEP(&ctx, now, &event, &observation) == MAC_ASSOCIATION_STATE);
                ...
                if (expected == MAC_ASSOCIATION_RESPONSE) {
                    ...
                    CHECK(memcmp(record.source_ieee, scenario == 0
                          ? (const uint8_t *)(golden + 13)
                          : (const uint8_t *)(body + (profile == 2 ? 15 : 13)), 8) == 0);
    ...
    CHECK(memcmp(&ctx, &saved, sizeof(ctx)) == 0);
    observation = 0xa5;
    CHECK(mac_association_step_rx(&ctx, now, NULL, &observation, 2) == MAC_ASSOCIATION_INVALID);
    CHECK(memcmp(&ctx, &saved, sizeof(ctx)) == 0 && observation == 0xa5);
    return 0;
}
...
static uint16_t receive_profiles(void)
{
    mac_frame_info_t frame, unchanged;
    mac_header_t header;
    unsigned form, n;
    uint32_t fcf;
    uint8_t *exact, length;
    uint8_t encoded[126], untouched[126];
    for (form = 0; form < 2; form++) {
        const uint8_t *wire = form ? explicit_pan : golden;
        unsigned size = form ? sizeof(explicit_pan) : sizeof(golden);
        for (fcf = 3; fcf <= 0xffff; fcf += 8) {
            memcpy(body, wire, size);
            body[0] = (uint8_t)fcf; body[1] = (uint8_t)(fcf >> 8);
            memset(&frame, 0xa5, sizeof(frame)); unchanged = frame;
            if (mac_frame_decode_profile(body, (uint16_t)size, &frame,
                                         MAC_RX_R22_ASSOCIATION_RESPONSE) == MAC_CODEC_OK) {
                CHECK(fcf == (form ? 0xcc23 : 0xcc63) || fcf == (form ? 0xcc33 : 0xcc73));
                CHECK(frame.payload_offset == size - 4 && frame.payload_length == 4);
                CHECK(frame.header.source_pan == 0x1234 && frame.header.destination_pan == 0x1234);
            } else {
                CHECK(fcf != (form ? 0xcc23 : 0xcc63) && fcf != (form ? 0xcc33 : 0xcc73));
                CHECK(memcmp(&frame, &unchanged, sizeof(frame)) == 0);
            }
        }
        for (n = 0; n <= 126; n++) {
            exact = malloc(n ? n : 1); CHECK(exact != NULL);
            memset(exact, 0xc7, n); memcpy(exact, wire, n < size ? n : size);
            memset(&frame, 0xa5, sizeof(frame)); unchanged = frame;
            CHECK((mac_frame_decode_profile(exact, (uint16_t)n, &frame,
                   MAC_RX_R22_ASSOCIATION_RESPONSE) == MAC_CODEC_OK) == (n == size));
            if (n != size) CHECK(memcmp(&frame, &unchanged, sizeof(frame)) == 0);
            free(exact);
        }
        CHECK(mac_frame_decode_profile(wire, (uint16_t)size, &frame,
              MAC_RX_R22_ASSOCIATION_RESPONSE) == MAC_CODEC_OK);
        header = frame.header;
        memset(encoded, 0xa5, sizeof(encoded)); memcpy(untouched, encoded, sizeof(encoded));
        length = 0xc7;
        CHECK(mac_frame_encode(&header, wire + size - 4, 4, encoded, sizeof(encoded), &length)
              == (form ? MAC_CODEC_INVALID_HEADER : MAC_CODEC_OK));
        if (form) {
            CHECK(length == 0xc7 && memcmp(encoded, untouched, sizeof(encoded)) == 0);
            CHECK(mac_frame_decode(wire, (uint16_t)size, &frame) == MAC_CODEC_INVALID_HEADER);
        } else {
            CHECK(length == size && memcmp(encoded, wire, size) == 0);
        }
        header.flags |= MAC_FLAG_PENDING;
        memcpy(untouched, encoded, sizeof(encoded)); length = 0xc7;
        CHECK(mac_frame_encode(&header, wire + size - 4, 4, encoded, sizeof(encoded), &length)
              == MAC_CODEC_INVALID_HEADER);
        CHECK(length == 0xc7 && memcmp(encoded, untouched, sizeof(encoded)) == 0);
        for (n = 0; n <= 0xffff; n++) {
            memcpy(body, wire, size);
            body[size - 3] = (uint8_t)n; body[size - 2] = (uint8_t)(n >> 8);
            CHECK((mac_frame_decode_profile(body, (uint16_t)size, &frame,
                   MAC_RX_R22_ASSOCIATION_RESPONSE) == MAC_CODEC_OK) == (n != 0xffff));
        }
        for (n = 0; n < 256; n++) {
            memcpy(body, wire, size);
            body[size - 3] = body[size - 2] = 0xff; body[size - 1] = (uint8_t)n;
            CHECK((mac_frame_decode_profile(body, (uint16_t)size, &frame,
                   MAC_RX_R22_ASSOCIATION_RESPONSE) == MAC_CODEC_OK) == (n == 1 || n == 2));
        }
        for (n = 0; n < 4; n++) {
            setup(); memcpy(body, wire, size); event.length = (uint16_t)size; event.stamp = now;
            if (n & 1) body[3] = body[4] = 0xff;
            if (n & 2) body[form ? 13 : 3] ^= 1;
            CHECK(mac_frame_decode_profile(body, (uint16_t)size, &frame,
                  MAC_RX_R22_ASSOCIATION_RESPONSE) == MAC_CODEC_OK);
            CHECK(mac_association_init(&ctx, now) == MAC_ASSOCIATION_OK);
            CHECK(mac_association_start(&ctx, &request, now) == MAC_ASSOCIATION_OK);
            CHECK(mac_association_step_rx(&ctx, now, &event, &observation,
                  MAC_RX_R22_ASSOCIATION_RESPONSE) == MAC_ASSOCIATION_OK);
            CHECK(observation == ((n == 0 || (form && n == 1))
                  ? MAC_ASSOCIATION_RESPONSE : MAC_ASSOCIATION_MISMATCH));
        }
    }
    for (n = 2; n < 256; n++) {
        memset(&frame, 0xa5, sizeof(frame)); unchanged = frame;
        CHECK(mac_frame_decode_profile(golden, sizeof(golden), &frame, (uint8_t)n)
        ...
        CHECK(mac_association_step_rx(&ctx, now, &event, &observation, (uint8_t)n)
...
int main(void)
{
    ...
    if (!line) line = receive_profiles();
    if (!line) line = exhaustive();
    if (line) {
        fprintf(stderr, "Association context failed C line %u (profile %u, scenario %u, observation %u, expected %u)\n",
                (unsigned)line, profile, scenario, observation, expected);
        return 1;
    }
    puts("Association context: 108 shared scenarios, 24576 FCFs, 6400 byte variants, "
         "3 x 65536 short addresses, 3 x 256 statuses, exact spans and finite/atomic cases PASS");
    return 0;
}
#else
