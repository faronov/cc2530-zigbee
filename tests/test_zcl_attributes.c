...
static const MCU_CODE uint8_t golden_request[] = {
    0, 0x5a, 0, 0x34, 0x12, 0x99, 0x99, 0x01, 0x30, 0, 0, 0, 0x20, 0x44, 0x44, 0x34, 0x12
};
static const MCU_CODE uint8_t golden_response[] = {
    0x18, 0x5a, 1, 0x34, 0x12, 0, 0x21, 0xef, 0xbe,
    0x99, 0x99, 0x86, 0x01, 0x30, 0x7e, 0, 0, 0, 0,
...
static uint16_t identifier_cases(void)
{
    ...
    for (manufacturer = 0; manufacturer < 2; manufacturer++) {
        for (i = 0; i < sizeof(identifiers) / sizeof(identifiers[0]); i++) {
            ...
            request[offset + 2u] = (uint8_t)identifiers[i];
            request[offset + 3u] = (uint8_t)(identifiers[i] >> 8);
            if (!manufacturer && (i == 1 || i == 2 || i == 5)) {
                CHECK(failure((uint16_t)(offset + 4u), 100, ZCL_CODEC_INVALID_TABLE) == 0);
                attributes[1].readable = 0;
                CHECK(failure((uint16_t)(offset + 2u), 100, ZCL_CODEC_INVALID_TABLE) == 0);
            } else {
                CHECK(zcl_read_attrs_unicast(&set, request, (uint16_t)(offset + 4u),
                                             response, 100, &info) == ZCL_CODEC_OK);
                CHECK(info.length == offset + 12u && info.requested_count == 2 && info.returned_count == 2);
                CHECK(response[offset] == 0x34 && response[offset + 1u] == 0x12 && response[offset + 2u] == 0);
                CHECK(response[offset + 6u] == request[offset + 2u] && response[offset + 7u] == request[offset + 3u]);
                CHECK(response[offset + 8u] == 0 && response[offset + 9u] == 0x21);
                CHECK(response[offset + 10u] == 0xef && response[offset + 11u] == 0xbe);
            }
            CHECK(attributes[0].id == 0x1234 && attributes[1].id == identifiers[i]);
            set.count = 1;
            CHECK(zcl_read_attrs_unicast(&set, request, (uint16_t)(offset + 4u),
                                         response, 100, &info) == ZCL_CODEC_OK);
            CHECK(info.length == offset + 9u && info.returned_count == 2);
            CHECK(response[offset + 6u] == request[offset + 2u] && response[offset + 7u] == request[offset + 3u]);
            CHECK(response[offset + 8u] == ZCL_STATUS_UNSUPPORTED_ATTRIBUTE);
        }
    }
    return 0;
}
