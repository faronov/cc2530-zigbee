...
static const MCU_CODE zcl_attribute_t code_attributes[] = {
    {0xffff, 1, {ZCL_TYPE_UINT16, 0, scalar, 2}},
    {0x1234, 0, {ZCL_TYPE_UINT8, 0xff, NULL, 65535}},
    {0x0000, 1, {ZCL_TYPE_NO_DATA, 0, NULL, 0}},
    {0x8000, 1, {ZCL_TYPE_CHARACTER_STRING, 1, NULL, 0}}
};
...
static uint16_t golden_cases(void)
{
    ...
    CHECK(response[4] == 0xff && response[5] == 0xff && response[6] == 0x21);
    request[5] = 0;
...
static uint16_t model_cases(void)
{
    ...
    for (i = 0; i < set.count; i++) {
        attributes[i].id = (uint16_t)(0xfff0u + (15u - i));
    ...
    for (i = 0; i < set.count; i++)
        CHECK(response[4u + 3u * i] == 0xf0u + i && response[5u + 3u * i] == 0xff && response[6u + 3u * i] == 0x20);
    CHECK(attributes[0].id == 0xffff && attributes[15].id == 0xfff0);
    base();
    for (i = 0; i < 3; i++)
