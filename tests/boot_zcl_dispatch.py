...
"""Execute isolated ZCL discovery and unicast dispatch tests without RF."""
...
def main():
    ...
    require(image.get(stop) == 0 and all(symbols.get(name) in image for name in (
        "_zcl_dispatch_unicast", "_zcl_read_attrs_unicast", "_zcl_frame_encode", "_zcl_frame_decode",
        "_zcl_value_encode", "_zcl_value_type_supported",
    )), "ZCL dispatcher/handlers/codecs/checkpoint are absent or changed")
    ...
    require(ram[STATUS_ADDRESS:STATUS_ADDRESS + 6] == b"ZCD1\x01\x08", "ZCL dispatch result ABI mismatch")
    failure = int.from_bytes(ram[STATUS_ADDRESS + 6:STATUS_ADDRESS + 8], "little")
    require(failure == 0, f"SDCC ZCL dispatch test failed at C source line {failure}")
    require(all(value == 0xa5 for address, value in enumerate(ram) if address not in allocated),
            "ZCL dispatch test wrote outside allocated XDATA")
    require(sfr[1] == symbols["s_SSEG"] - 1 and iram[128:] == b"\xc7" * 128,
            "ZCL dispatch test crossed the upper IRAM guard or failed to unwind")
    print(f"ZCL discovery/dispatch: {len(image)} CODE bytes; strict linked layout, actual vectors "
