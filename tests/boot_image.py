...
def check_artifact_rejections(output, board, image_name="bringup"):
    ...
    with tempfile.TemporaryDirectory(prefix="cc2530-m0-artifacts-") as directory:
        ...
        if image_name in ("radio_noise_fixture", "radio_link_fixture"):
            for listing in output.glob(f"{image_name}.*.rst"):
...
def main():
    ...
    elif args.image == "radio_link_fixture":
        from boot_radio_link_fixture import check_radio_link_fixture
