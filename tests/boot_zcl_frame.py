...
"""Execute the standalone ZCL header and APS composition tests without RF."""
...
def main():
    ...
    path = args.output / "zcl_frame_test.ihx"
    ...
    allocated = verify_component_layout(
        ...
        "zcl_frame_test_result", ("zcl_frame.c", "aps_frame.c", "test_zcl_frame.c"),
    )
    stop = symbols["_zcl_frame_test_done"]
    require(image.get(stop) == 0
            and all(symbols.get(f"_{layer}_frame_{operation}") in image
                    for layer in ("zcl", "aps") for operation in ("encode", "decode")),
            "ZCL/APS codec/checkpoint is absent or changed")
    ...
    require(ram[STATUS_ADDRESS:STATUS_ADDRESS + 6] == b"ZCF1\x01\x08", "ZCL frame result ABI mismatch")
    failure = int.from_bytes(ram[STATUS_ADDRESS + 6:STATUS_ADDRESS + 8], "little")
    require(failure == 0, f"SDCC ZCL frame test failed at C source line {failure}")
    require(all(value == 0xa5 for address, value in enumerate(ram) if address not in allocated),
            "ZCL frame test wrote outside allocated XDATA")
    require(sfr[1] == symbols["s_SSEG"] - 1 and iram[128:] == b"\xc7" * 128,
            "ZCL frame test crossed the upper IRAM guard or failed to unwind")
    print(f"ZCL/APS frames: {len(image)} CODE bytes; strict linked layout, actual vectors "
