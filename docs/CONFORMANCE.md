...
| MAC-TX-01 | One copied unsecured DATA or canonical Beacon Request, unslotted NB/BE backoff, bounded CCA/retries, legacy ACK/DSN matching and explicit lifetime/quiescence ownership | IEEE 802.15.4-2006 selected O-QPSK subset; [primary derivations and adapter contract](MAC_TX.md) | Offline state machine using the real codec / host-tested, image-checked, simulated; abstract symbol time, no active scan, radio adapter or board linkage | M3; unified radio ownership, physical ACK/timing and full-stack resources remain open |
