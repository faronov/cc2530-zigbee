...
The remaining #9 audit/calibration scope is now consolidated into #4; the
administrative closure supplies no new hardware evidence and does not close
M2 or #8. Entropy remains #10. Further physical work
...
| SEC-KHASH-01 | Fixed-key/purpose HMAC-AES-MMO: transport00/load02 and initiator Verify-Key03; no partial output or use of03 result as an encryption key | R22 B.1.4 p490,4.5.3 pp457-458,4.4.10.7.4 p452; [contract](KEY_HASH.md) | Genuine AES/MMO, independent public KATs, inner/outer failures; host-tested, image-checked, simulated | #23 cryptographic prerequisite only; no key slots/replay/TC confirmation, entropy, provisioning or membership |
| SEC-RESIDENT-01 | All thirteen real crypto/NV services resident with unchanged original callers and checked active DATA-frame sharing | SDCC4.2.0 emitted objects and TI SWRU191F CPU; [contract](SECURITY_RESIDENT.md) | Complete image/ABI/libc/frame proof and original service simulation, no relaxed old caps | #23 memory-layout prerequisite only; no mixed lifecycle, key/replay/TC admission, board linkage, ISR or whole-stack fit |
| SEC-KEY-01 | Fixed direct-parent/TC two-slot key and durable replay state, old/pending/current TC phases, authenticated Transport/Verify/Confirm and explicit provisioning/Leave history | Pinned R22/BDB3.0.1 [owner contract and primary clauses](SECURITY_KEYS.md); [banked composition](BANKED_SECURITY.md) | Real crypto/counter/journal/flash; host-tested plus complete banked image/ABI/physical-DATA/libc/alias proof and mixed8051 lifecycle simulation | Bounded #23 implementation accepted; whole MCU join #26, secure recovery #27, entropy #10, electrical durability #8, physical coordinator #28 and conformance review remain open; no general peer/TC migration or hardware claim |
