...
class LayoutTests(unittest.TestCase):
    ...
    def test_isolated_security_cannot_enter_board_images(self):
        for image in IMAGES:
            for name in ("_ccm_star_crypt", "_zigbee_security_crypt", "_zigbee_security_inspect", "_security_done",
                         ...
                         "_security_iram_low", "_security_iram_high"):
    ...
    def test_isolated_mac_radio_cannot_enter_board_images(self):
        for image in IMAGES:
            for name in ("_mac_radio_init", "_mac_radio_now", "_mac_radio_receive", "_mac_radio_send",
                         "_mac_radio_stop", "_mac_radio_resume", "_mac_radio_diagnostic",
                         "_mac_radio_shared_end", "_mac_radio_reserved_end", "_mac_radio_test_result"):
                with self.subTest(image=image, name=name), self.assertRaisesRegex(ValueError, "MAC radio"):
                    verify_layout(self.symbols | {name: 0x100}, self.memory, self.debug, image)
            for source in ("mac_radio.c", "test_mac_radio.c"):
                with self.subTest(image=image, source=source), self.assertRaisesRegex(ValueError, "MAC radio"):
                    verify_layout(self.symbols, self.memory, self.debug+f"\nC${source}$1", image)

    def test_isolated_mac_stamp_cannot_enter_board_images(self):
        for image in IMAGES:
            for name in ("_mac_stamp_project", "_mac_stamp_test_result"):
                with self.subTest(image=image, name=name), self.assertRaisesRegex(ValueError, "MAC stamp"):
                    verify_layout(self.symbols | {name: 0x100}, self.memory, self.debug, image)
            for source in ("mac_stamp.c", "test_mac_stamp.c"):
                with self.subTest(image=image, source=source), self.assertRaisesRegex(ValueError, "MAC stamp"):
                    verify_layout(self.symbols, self.memory, self.debug+f"\nC${source}$1", image)
