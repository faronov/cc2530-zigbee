...
execution proof for #51. It does not supply an acquisition/programming runner,
...
| `library/lib/libjaylink.so.0.2.0` | Genuinely source-built libjaylink 0.3.1 (library ABI filename is not its package version) |
...
| `library/lib/libjaylink.so.0.2.0` | `bd22b0246f79c15fe7fd067e33cd4f9506bdacb745a1fe698a4c6f75d581ec87` |
...
binds 2,041 source files and the actual loader-resolved runtime closure,
...
locked 0.3.1 archive, **not** the unused OpenOCD libjaylink submodule.
...
Every synthetic process replaces the sole adapter DSO with the test library.
...
programmer. It runs four tests and explicitly skips the external linked proof:
...
workspace to execute all five tests, including all 58 linked sequences:
...
compares dependency source files to the locked archives, verifies downloaded
...
host kernel or probe firmware. Keep the reviewed environment and identities
...
env -i PATH=/usr/bin:/bin LC_ALL=C HOME=/nonexistent \
  LD_LIBRARY_PATH="$REVIEW/library/lib:$REVIEW/deps/usr/lib/x86_64-linux-gnu" \
