...
IMAGES = ("bringup", "debug_fixture", "timebase_fixture", "clock_fixture", "irq_fixture",
          ...
          "flash_fixture", "radio_tx_fixture", "radio_noise_fixture", "radio_link_fixture")
CAPABILITIES = {
    ...
    "radio_link_fixture": "non-networking-boot-disarmed-same-owner-radio-tx-rx-fixture",
}
...
def verify_layout(symbols, memory, debug, image_name="bringup"):
    ...
    require(not any(name.startswith(("_radio_autoack_test", "_host_radio_autoack")) for name in symbols) and
            not any(f"C${name}$" in debug for name in
                    ("test_radio_autoack.c", "host_radio_autoack.c", "test_radio_link_fixture.c")),
            "Board image must not link receiver AUTOACK tests/models")
    require(image_name == "radio_link_fixture" or
            (not any(name.startswith(("_radio_autoack_", "_radio_link_fixture_")) for name in symbols) and
             not any(f"C${name}$" in debug for name in
                     ("radio_autoack.c", "radio_link_fixture.c", "radio_link_fixture_state.c"))),
            "Board image must not link the isolated receiver AUTOACK owner")
    ...
    require(not any(name.startswith("_mac_poll_") for name in symbols) and
            not any(f"C${name}$" in debug for name in ("mac_poll.c", "test_mac_poll.c", "host_mac_poll.c")),
            "Board image must not link the isolated MAC poll extraction controller")
    ...
    require(not any(name.startswith("_mac_stamp_") for name in symbols) and
            not any(f"C${name}$" in debug for name in ("mac_stamp.c", "test_mac_stamp.c")),
            "Board image must not link the isolated MAC stamp projection")
    ...
    require(not any(name.startswith("_zdo_node_") for name in symbols) and
            not any(token in debug for token in (
                "C$zdo_node.c$", "C$test_zdo_node.c$", "G$zdo_node_",
                "Fzdo_node$", "Lzdo_node.", "Ftest_zdo_node$", "Ltest_zdo_node.",
    ...
    require(not any(name.startswith(("_ccm_star_", "_zigbee_security_", "_security_", "_counter_",
                                    "_zigbee_mmo_", "_install_code_", "_mmo_")) for name in symbols) and
            not any(token in debug for module in ("ccm_star", "zigbee_security", "test_zigbee_security",
                                                 "security_counter", "test_security_counter",
                                                 "zigbee_mmo", "test_zigbee_mmo")
            ...
            not any(token in debug for token in ("G$ccm_star_", "G$zigbee_security_", "G$security_", "G$counter_",
                                                "G$zigbee_mmo_", "G$install_code_", "G$mmo_")),
    ...
    require(not any(name.startswith(("_zcl_wr_", "_zcl_write_")) for name in symbols) and
            not any(token in debug for token in (
                "C$zcl_write.c$", "C$test_zcl_write.c$", "G$zcl_wr_", "G$zcl_write_",
                "Fzcl_write$", "Lzcl_write.", "M:zcl_write",
            )),
            "Board image must not link the isolated ZCL write path")
    ...
    budget = (1024 if image_name in ("radio_rx_fixture", "radio_link_fixture") else
              768 if image_name == "radio_noise_fixture" else 512)
...
def verify_artifacts(output, board, image_name="bringup"):
    ...
    elif image_name == "radio_link_fixture":
        from radio_link_fixture import modules, verify_fixture, verify_listings, verify_memory
        verify_fixture(image, symbols, debug, board)
        listings = {name: (output / f"radio_link_fixture.{name}.rst").read_text()
                    for name in modules(board)}
        verify_listings(listings, image, symbols, board)
        verify_memory(memory)
    flash = re.search(
