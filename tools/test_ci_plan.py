...
class SelectionTests(unittest.TestCase):
    ...
    def test_full_preserves_110_workers_and_adds_two_workspace_workers(self):
        selected = plan.full_plan("test")
        rows = selected["matrix"]["include"]
        self.assertEqual(len(rows), 112)
        self.assertEqual(len({r["name"] for r in rows}), 112)
    ...
    def test_shared_and_unknown_inputs_fail_safe_to_full(self):
        for path in ("Makefile", ".github/workflows/ci.yml", "include/bdb_join.h",
            ...
            with self.subTest(path=path):
                ...
                self.assertEqual(len(selected["matrix"]["include"]), 112)
                self.assertEqual(selected["campaign"], "full")
    ...
    def test_actual_shared_c_consumers_not_filename_matching(self):
        cases = {
            "src/bdb_join.c": {"ed-bdb-join", "mac-link", "mac-link-ram", "mac-link-workspace"} | {name for name in plan.COMPONENTS if name.startswith("banked-join-")},
            "tests/bdb_join_layout.c": {"ed-bdb-join"},
            "src/security_keys.c": {"ed-security-keys", "ed-bdb-join", "banked-security", "mac-link", "mac-link-ram", "mac-link-workspace"} |
            ...
            "tests/test_mac_handoff.c": {"mac-handoff", "mac-adapter", "mac-reconfig", "mac-link", "mac-link-ram", "mac-link-workspace"},
            "src/mac_adapter.c": {"mac-adapter", "mac-reconfig", "mac-link", "mac-link-ram", "mac-link-workspace"},
            "tests/test_mac_observed.c": {"mac-adapter"},
            "tests/test_mac_adapter.c": {"mac-adapter", "mac-reconfig", "mac-link", "mac-link-ram", "mac-link-workspace"},
            "tests/test_mac_reconfig.c": {"mac-reconfig"},
            "tests/test_mac_link_scan.c": {"mac-link"},
            "tests/test_mac_link_join.c": {"mac-link", "mac-link-ram", "mac-link-workspace"},
            "tests/test_mac_link_e2e.c": {"mac-link", "mac-link-ram", "mac-link-workspace"},
            "tests/mac_link_peer.c": {"mac-link", "mac-link-ram", "mac-link-workspace"},
            "src/mac_link_driver.c": {"mac-link", "mac-link-ram", "mac-link-workspace"},
            "tests/mac_adapter_fixture.c": {"mac-adapter", "mac-reconfig", "mac-link", "mac-link-ram", "mac-link-workspace"},
            "tests/test_mac_link_ram.c": {"mac-link-ram", "mac-link-workspace"},
            "tests/mac_link_ram_layout.c": {"mac-link-ram", "mac-link-workspace"},
            "src/mac_link_workspace.c": {"mac-link-workspace"},
            "tests/test_mac_link_workspace.c": {"mac-link-workspace"},
